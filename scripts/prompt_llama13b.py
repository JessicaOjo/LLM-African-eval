...
def load_llama(model_name):
    ...
    model = LlamaForCausalLM.from_pretrained(model_name, cache_dir="/SAN/intelsys/llm/dadelani/models",
                                             token="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")

    tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir="/SAN/intelsys/llm/dadelani/models",
                                              token="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")
...
'''
def prompt_llama(model, tokenizer, text):
...
def sentiment(model_pipeline, tokenizer, output_dir):
    ...
    for file in files:
        ...
        for index in range(df.shape[0]):
            ...
            input_mes = input_prompt + message +' [/INST] '
        ...
        df['llama'] = completions
        df.to_csv(output_dir + language + '.tsv', sep='\t')

        #f1 = f1_score(df['label'], df['llama'], average='weighted')
        #metric[language] = f1

    #with open('sentiment_llama.json', 'w') as outfile:
...
def news_classification(model_pipeline, tokenizer, output_dir):
    ...
    for file in files:
        ...
        #    df['llama'][df['llama'].str.contains(label.lower())] = label

        #f1 = f1_score(df['category'], df['llama'], average='weighted')
        #metric[lang] = f1
...
def main(senti: bool = False,
    ...
    if senti is True:
        ...
        output_dir = 'results_llama13b/mt_german/'
        create_dir(output_dir)
        ...
        output_dir = 'results_llama13b/ner/'
        create_dir(output_dir)
